//we contain all ads
//could be that this class also exists in other parts, not only MediaPlayer, do not want that ad duplicate (same ad in video and other site) - singleton, each object will have a different ad
//this ads will be like in youtube like a small banner in down part

export interface Ad {
  imageUrl: string;
  title: string;
  body: string;
  //if user clicks on Ad takes him to url
  url: string;
}
...
class Ads {
  ...
  //public method
  static getInstance() {
    //will check if we have already an instance - if we do not, create a new one
    if (!Ads.instance) {
  ...
  //we need an initial list of ads
  private initAds() {
    //destructuring, leave ALL_ADS intact
    //add a copy of the array in a new array
    this.ads = [...ALL_ADS];
  }

  //give ads to those who ask for it
  getAd() {
    if (this.ads.length === 0) {
      //start over: initialize them again
      this.initAds();
    }
    //pop takes a value out of the array and will return it
    return this.ads.pop();
  }
}
